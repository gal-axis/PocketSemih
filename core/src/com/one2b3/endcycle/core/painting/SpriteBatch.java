...
public class SpriteBatch implements Batch {
	...
	// Can be uncommented to enable texture caching
	private final TextureBinder binder = null; // newDefaultTextureBinder(DefaultTextureBinder.LRU);
	...
	public int getReuseCount() {
		return binder == null ? 0 : binder.getReuseCount();
	}
	...
	private boolean cacheTextures() {
		return binder != null && textureSize != -1;
	}
	...
	public void flush() {
		...
		int active = 0;
		if (cacheTextures()) {
			active = binder.bind(lastTexture);
		} else {
			lastTexture.bind();
		}
		if (u_texture != -1) {
			shader.setUniformi(u_texture, active);
		}
		if (textureSize != -1) {
			shader.setUniformf(textureSize, lastTexture.getWidth(), lastTexture.getHeight());
		}
		Mesh mesh = this.mesh;
	...
	public void updateUniforms(ShaderProgram shader) {
		textureSize = shader.getUniformLocation("TextureSize");
		if (textureSize != -1 && lastTexture != null) {
			getShader().setUniformf(textureSize, lastTexture.getWidth(), lastTexture.getHeight());
		}
